  ...
  const toggleSuspend = async (m: any) => {
    ...
    await supabase.from("notifications").insert({
      ...
      message: newStatus === "suspended"
        ? "Your Remuneration Portal account has been suspended. Please contact support."
        : "Your Remuneration Portal account has been reinstated.",
      type: "account",
    });
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        <div>
          <h1 className="font-heading text-2xl md:text-3xl font-bold text-foreground">Remuneration Members</h1>
          <p className="text-muted-foreground mt-1">
            {members.length} member{members.length !== 1 ? "s" : ""} registered on the Remuneration Portal.
          </p>
        </div>
        ...
        {loading ? (
          ...
          <Card className="shadow-card">
            <CardContent className="p-8 text-center">
              ...
              <p className="text-muted-foreground">No remuneration members found.</p>
            </CardContent>
          </Card>
        ) : (
